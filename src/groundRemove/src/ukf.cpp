...
void UKF::updateMeasurementForCTRV(const std::vector<BBox>& object_vec)
{
...
void UKF::updateForCTRV()
{
  ...
  else
  {
    ...
    if (debugBool)
    {
		...
		std::cout << "updateFor CTRV:\n" << p_ctrv_  << std::endl << k_ctrv_ << std::endl << s_ctrv_ << std::endl << std::flush;
    }
	p_ctrv_ = p_ctrv_ - k_ctrv_ * s_ctrv_ * k_ctrv_.transpose(); 
	if (debugBool)
	{
		std::cout << "after update:\n";
		std::cout << "updateFor CTRV:\n" << p_ctrv_ << std::endl << std::flush;
	}
    x_merge_.col(0) = x_ctrv_.col(0);
  }
}

void UKF::updateSUKF(const std::vector<BBox>& object_vec)
{
 ...
 ****************************************************************************/
  ...
  ****************************************************************************/
  ...
  // 对称正定的矩阵表示成一个下三角矩阵L和其转置的乘积的分解， 为了开根号
  ...
  ****************************************************************************/
  ...
  if (debugBool)
  {
    std::cout << "llt matrixL L:\n";
    std::cout << L << std::endl;

    std::cout << "x_sig:\n";
    std::cout << x_sig << std::endl;

    std::cout << "x_sig_pred:\n";
    std::cout << x_sig_pred << std::endl;
    
    std::cout << "weights_s_ : " << std::endl << weights_s_.transpose() << std::endl;
  ...
  {  // iterate over sigma points
    // state difference
    // x 为求去的均值， 直接都是预测的均值 和 其他预测的值求方差， 
    // 此处暂时还没有用到测量
...
double UKF::calculateNIS(const BBox& in_object, const int motion_ind)
{
...
bool UKF::isLaneDirectionAvailable(const BBox& in_object, const int motion_ind,
...
void UKF::checkLaneDirectionAvailability(const BBox& in_object,
