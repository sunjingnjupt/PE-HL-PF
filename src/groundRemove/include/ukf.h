...
public:
  ...
  void updateMeasurementForCTRV(const std::vector<BBox>& object_vec);

  void updateForCTRV();

  void updateEachMotion(const double detection_probability, const double gate_probability, const double gating_threshold,
                        const std::vector<BBox>& object_vec, std::vector<double>& lambda_vec);

  void updateSUKF(const std::vector<BBox>& object_vec);
  ...
  void checkLaneDirectionAvailability(const BBox& in_object,
  ...
  double calculateNIS(const BBox& in_object, const int motion_ind);

  bool isLaneDirectionAvailable(const BBox& in_object, const int motion_ind,
